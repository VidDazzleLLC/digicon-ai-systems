...
// Database Migration Script for Code Execution Architecture
// Creates all necessary tables from schema-code-execution.prisma
//
// Usage: node scripts/migrate-code-execution.js
...
const DB_CONFIG = {
  ...
  database: process.env.DB_NAME || 'digicon',
...
const MIGRATIONS = {
  ...
  // Create GeminiFileIndex table
  createGeminiFileIndex: `
...
async function migrate() {
  console.log('🚀 Starting Code Execution Architecture Database Migration');
  console.log('=' .repeat(60));

  try {
    // Note: In production, use pg or prisma client
    console.log('\n📋 Migration SQL Generated:');
    console.log('=' .repeat(60));
    
    Object.entries(MIGRATIONS).forEach(([name, sql]) => {
      console.log(`\n✓ ${name}`);
      console.log(sql.trim().substring(0, 150) + '...');
    });

    console.log('\n' + '='.repeat(60));
    console.log('✅ Migration script prepared successfully!');
    console.log('\n📝 Next Steps:');
    console.log('   1. Configure your PostgreSQL connection');
    console.log('   2. Run the SQL statements above in your database');
    console.log('   3. Or use Prisma: prisma migrate dev');
    console.log('   4. Run seed script: node scripts/seed-tools.js');
    
    // Write to file
    const migrationSQL = Object.values(MIGRATIONS).join('\n\n');
    const outputPath = path.join(__dirname, '..', 'migrations', 'code-execution-schema.sql');
