        ...
        if (session.metadata?.conferenceRoomId) {
          ...
          console.log(`✅ CONFERENCE ROOM ${session.metadata.conferenceRoomId} STATUS UPDATED TO PAID`);
              ...
              // Handle Audit Payment
                        ...
                        const auditRequest = await getAuditRequest((session.metadata as any)?.auditRequestId);
                        ...
                        // Mark as paid
                        await updateAuditRequest((session.metadata as any)?.auditRequestId, { status: 'paid' });
                        ...
                        await updateAuditRequest((session.metadata as any)?.auditRequestId, {
                                    status: 'report_ready'
                                              });
